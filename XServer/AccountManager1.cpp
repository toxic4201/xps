...
#include <C:\Users\Mary Dull\Desktop\xenimus\xps\XServer\Shared.h>
...
bool AccountManager::CreatePlayer(Account* a, std::string Name, uint16 Str, uint16 Agi, uint16 Cons, uint16 Int, uint16 Wis, uint16 Path, uint16 Gender, uint16 Race, uint16 Class)
{
	...
	{
		SendAccountMessage(a, ACCOUNT_MSG_NAME_ALREADY_EXIST); //name already exists
		return false;
	}
	...
	SendAccountMessage(a, ACCOUNT_MSG_PLAYER_WAS_SAVED_ON_SERVER);
...
void AccountManager::SendAccountMessage(Account* a, uint32 message)
{
	if (a == NULL)
		return;
	...
	p.m_addr = a->netinfo;
	sGameClientSocket.SendPacket(&p);
}
...
void Account::SendActionResult( uint16 res )
{
	...
	data << res;
	data.m_addr = netinfo;
	sGameClientSocket.SendPacket(&data);
}
...
void Account::SendCharList()
{
	...
	if (m_data->banexpiretime >= time(NULL))
	{
		...
		p2.WriteTo(200);
		p2.m_addr = netinfo;
		sGameClientSocket.SendPacket(&p2);
